...
class MainController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('frontend.index', ['home_page_sections' => $HomepageSection, 'top_companies' => $TopCompanyList, 'random_selected_company' => $RandomSelectedComment, 'last_comment' => $LastComment, 'last_salary_comment' => $LastSalaryComment, 'setting' => '', 'ratingcount' => 5]);
    }
    ...
    public function categoryNames(Request $request)
    {
        ...
        if ($request->title != '') {
            $Title = str_replace(',', '', $request->title);
            $Title1 = str_replace('.', '', $request->category);

            $CategoryList = Category::where(function ($query) use ($char, $char1, $Title, $Title1) {
                $query->where(\DB::raw("REPLACE(title, '$char', '')"), 'like', '%' . $Title . '%')
                    ->orWhere(\DB::raw("REPLACE(title, '$char1', '')"), 'like', '%' . $Title1 . '%');
            })->where('status', 1)->select('id as id', 'title as text')->get();
        } else {
            $CategoryList = array();
        }
    ...
    public function companyList(Request $request)
    {
        ...
        return view('frontend.company_list', ['list' => $CompanyList]);
    }
    ...
    public function singleCompany($slug)
    {
        ...
        if (isset($CompanyDetails->comments)) {
            ...
            return view('frontend.company_details', ['company_details' => $CompanyDetails, 'ratingcount' => 5]);
        } else {
            return view('500');
        }
    }
